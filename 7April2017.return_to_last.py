...
rospy.init_node("return_to_last_test")
...
def initialize():
	...
	set_transform("map","last_point",trans,rot)
...
with open('test_1_odom_only_return_to_last.csv', 'ab') as csvfile:
	...
	for i in range(1,20):
		...
		(ta,ra) = get_transform("map","last_point")
		(t4,r4) = get_transform("origin","last_point")
		e4 = tf.transformations.euler_from_quaternion(r4)
		print "Returning to last point"
		print 'point_number:',i,'point_type: last_point x:',t4[0],'y:',t4[1],'yaw:',e4[2],""
		move_base_client(ta,ra)
		time.sleep(3)
		check_camera()
		(t5,r5) = get_transform("origin","pioneer")
		e5 = tf.transformations.euler_from_quaternion(r5)
		writer.writerow({'point_number': i, 'point_type': 'camera_at_last_point', 'x': t5[0], 'y': t5[1], 'yaw': e5[2]})
		print 'point_number:',i,'point_type: camera_at_last_point x:',t5[0],'y:',t5[1],'yaw:',e5[2],""
		(t6,r6) = lookup_odom()
		e6 = tf.transformations.euler_from_quaternion(r6)
		writer.writerow({'point_number': i, 'point_type': 'odom_at_last_point', 'x': t6[0], 'y': t6[1], 'yaw': e6[2]})
		print 'point_number:',i,'point_type: odom_at_last_point x:',t6[0],'y:',t6[1],'yaw:',e6[2],""
		set_transform("origin","last_point",t6,r6)